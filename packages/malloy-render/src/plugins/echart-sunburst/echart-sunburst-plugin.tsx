...
export const EchartSunburstPluginFactory: RenderPluginFactory<DOMRenderPluginInstance> =
  {
    name: 'echart_sunburst',

    getValidationSpec: (): RendererValidationSpec => ({
      renderer: 'echart_sunburst',
      ownedPaths: [
        ['echart_sunburst'],
        ['echart_sunburst', 'colors'],
        ['echart_sunburst', 'echarts'],
      ],
    }),

    matches: (field: Field, fieldTag: Tag, fieldType: FieldType): boolean => {
      const hasTag = fieldTag.has('echart_sunburst');
      if (hasTag && fieldType !== FieldType.RepeatedRecord) {
        throw new Error(
          'EChart Sunburst: field must be a nested query (repeated record).'
        );
      }
      return hasTag;
    },
    ...
    create: (field: Field, pluginOptions?: unknown): DOMRenderPluginInstance => ({
      ...
      renderToDOM: (container: HTMLElement, props: RenderProps): void => {
        if (!props.dataColumn.isRepeatedRecord()) {
          throw new Error('EChart Sunburst: data is not a repeated record');
        }
        ...
        if (!dimField || !measureField) {
          throw new Error(
            'EChart Sunburst: requires at least 1 dimension and 1 measure'
          );
        }
        ...
        const pluginTag = field.tag.tag('echart_sunburst');
      ...
      getMetadata: () => ({type: 'echart_sunburst', field}),
    }),
  };
